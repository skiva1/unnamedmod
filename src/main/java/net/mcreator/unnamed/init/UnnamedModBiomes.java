...
import net.mcreator.unnamed.world.biome.SolarDesertBiome;
...
public class UnnamedModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, UnnamedMod.MODID);
	public static final RegistryObject<Biome> SOLAR_DESERT = REGISTRY.register("solar_desert", SolarDesertBiome::createBiome);
		...
		for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					parameters.add(new Pair<>(SolarDesertBiome.PARAMETER_POINT,
							biomeRegistry.getOrCreateHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, SOLAR_DESERT.getId()))));
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						...
						surfaceRules.add(1,
								preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, SOLAR_DESERT.getId()),
										UnnamedModBlocks.SOLAR_SAND.get().defaultBlockState(), UnnamedModBlocks.SOLAR_SAND.get().defaultBlockState(),
										UnnamedModBlocks.SOLAR_SAND.get().defaultBlockState()));
